...
@section('content_header')
    <h1><b>Prescripciones / Editar prescripción</b></h1>
    <hr>
@stop
...
<div class="row">
    <div class="col-md-12">
        <div class="card card-primary">
            ...
            <div class="card-body">
                <form action="{{ route('admin.prescripciones.update', $prescripcion->id) }}" method="post">
                    @csrf
                    @method('PUT')
                                ...
                                <select name="historial_id" class="form-control" required>
                                    ...
                                    @foreach($historiales as $historial)
                                        <option value="{{ $historial->id }}"
                                            {{ $prescripcion->historial_id == $historial->id ? 'selected' : '' }}>
                                ...
                                <select name="medicamento_id" class="form-control" required>
                                    ...
                                    @foreach($medicamentos as $medicamento)
                                        <option value="{{ $medicamento->id }}"
                                            {{ $prescripcion->medicamento_id == $medicamento->id ? 'selected' : '' }}>
                                            {{ $medicamento->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="dosis" class="form-control" value="{{ old('dosis', $prescripcion->dosis) }}" placeholder="Ingrese dosis..." required>
                                @error('dosis')
                                    <small style="color: red">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="observaciones" class="form-control" value="{{ old('observaciones', $prescripcion->observaciones) }}" placeholder="Ingrese observaciones...">
                                @error('observaciones')
                                    <small style="color: red">{{ $message }}</small>
                                @enderror
                            </div>
